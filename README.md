...
| **`pi-ac-verifier`** | GPT-5.5 (`openai-codex`) · high | read, grep, find, ls, **bash** + linear + `github_pr_comment` | **Runs** dual-source AC verification (Linear checkbox AC + PR body AC/checklist) against the PR's actual head commit with constrained tests/build/inspection evidence; fails if validation dirties the worktree; posts PR + Linear evidence itself; checks boxes only on real pass. No write/edit tools, no pushes, no PR mutation beyond comments; never implementer/lead/code-writer for that PR. |
