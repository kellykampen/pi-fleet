...
validation, `castJob` dispatch for `profile: "reviewer"`, token-precedence
resolution, the generated runner script (asserts the read-only gh calls are
present and no code-mutating command — `git push`/`git commit`/`git
checkout`/`gh repo clone`/`gh pr merge`/`gh pr review` — ever appears), the
reviewer result finalizer (succeeded / PR-fetch-failed / comment-post-failed /
reviewer-process-failed paths), and `reconnectSandbox`/`refreshFromSandbox`
carrying the reviewer-only result fields.

This PR does **not** include a live E2B sandbox run against a real PR — no
`E2B_API_KEY`/`FLEET_GITHUB_TOKEN` credentials were available in the
implementer's environment. A real remote reviewer cast against an actual PR,
producing an actual posted comment, remains an open gate before this ticket's
