...
**`fleet-mail` is the DEFAULT fleet communication channel.** Status uplink uses the
**`fleet-mail` CLI** (shared file inbox under `~/.pi-fleet/mail`).
Do **not** drip mid-task status via terminal multiplexor steers into the project lead.
cmux/send is only for **launch / bootstrap / emergency** — not routine agent-to-agent status.
...
- **Topology:** workers/reviewers/AC-verifiers → **project-lead only** (never conductor).
  ...
  Mailbox == cmux pane/tab name. Path: worker → lead → conductor.
- **Anti-spam:** `type=status` requires `--ticket`; a new status **replaces** the prior unacked
  status for the same sender+ticket.
- Lead pulls with `fleet-mail inbox --mailbox <workspace>-project-lead --unread` on **startup**,
  every **task boundary**, every **5–10 min**, and **before reporting blocked or done**, then acks.
