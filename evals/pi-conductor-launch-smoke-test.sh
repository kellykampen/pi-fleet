#!/usr/bin/env bash
# Deterministic launch-boundary smoke for the isolated Pi conductor policy overlay.
set -euo pipefail
...
for config in \
  ...
  if [ -L "$config" ] && [ "$(readlink "$config")" = "$DIR/permission-system/conductor.json" ]; then
    ok "$config pins tracked conductor policy"
  else
    no "$config pins tracked conductor policy"
  fi
done
...
out=$(cd "$CALLER" && env HOME="$FAKE_HOME" PI_CODING_AGENT_DIR="$SOURCE_AGENT" \
  FLEET_CONDUCTOR_RUNTIME_DIR="$FALLBACK_RUNTIME" PATH="$FAKE_BIN:$PATH" \
  "$DIR/bin/pi-conductor" --print hi)
