...
"$ROOT/bin/pi-fleet-state-migrate" --apply >/dev/null & p1=$!
"$ROOT/bin/pi-fleet-state-migrate" --apply >/dev/null & p2=$!
wait "$p1"; wait "$p2"
[[ "$(cat "$PI_FLEET_HOME/state/e2b/jobs/conflict.json")" == destination ]]
python3 -m json.tool "$manifest" >/dev/null
mkdir "$PI_FLEET_HOME/state/migrations/.migration.lock"; SECONDS=0
! "$ROOT/bin/pi-fleet-state-migrate" --apply >/dev/null 2>&1
[[ "$SECONDS" -le 7 ]]; rmdir "$PI_FLEET_HOME/state/migrations/.migration.lock"

# Rollback rejects manifest symlinks, lexical traversal, dot components, and physical escapes.
cp "$manifest" "$TMP/good-manifest"
rm "$manifest"; ln -s "$TMP/good-manifest" "$manifest"
! "$ROOT/bin/pi-fleet-state-migrate" --rollback >/dev/null 2>&1
rm "$manifest"; cp "$TMP/good-manifest" "$manifest"; chmod 600 "$manifest"
...
mkdir -p "$TMP/outside" "$PI_FLEET_HOME/archive"; rm -rf "$PI_FLEET_HOME/archive/legacy-root"; ln -s "$TMP/outside" "$PI_FLEET_HOME/archive/legacy-root"
python3 - "$manifest" "$PI_FLEET_HOME/archive/legacy-root/escape" <<'PY'
import json,sys
json.dump({"version":1,"created":[{"source":"legacy","destination":sys.argv[2],"sha256":"0"*64}]},open(sys.argv[1],"w"))
PY
! "$ROOT/bin/pi-fleet-state-migrate" --rollback >/dev/null 2>&1
rm "$PI_FLEET_HOME/archive/legacy-root"; cp "$TMP/good-manifest" "$manifest"
...
mkdir -p "$TMP/physical-parent/fleet/state"; ln -s "$TMP/physical-parent" "$TMP/ancestor-link"
