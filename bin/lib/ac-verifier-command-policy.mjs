const READ_UTILITIES = new Set(["cat", "ls", "grep", "rg", "head", "tail", "wc", "find", "jq"]);
const READ_ONLY_GIT = new Set(["status", "log", "diff", "show", "rev-parse"]);
const UNSAFE_FIND_ACTIONS = new Set(["-delete", "-exec", "-execdir", "-ok", "-okdir", "-fls", "-fprint", "-fprint0", "-fprintf"]);
const TEST_RUNNERS = new Set(["pnpm", "npm", "npx", "node", "tsx", "vitest", "jest", "mocha", "pytest"]);
...
function tokenizeAtomicCommand(command) {
  if (typeof command !== "string" || command.trim() === "") return { error: "empty command" };
  if (command.includes("$(") || command.includes("`")) return { error: "command substitution is not allowed" };
  ...
  for (let index = 0; index < command.length; index += 1) {
    ...
    if (/\s/u.test(char)) {
      if (char === "\n" || char === "\r") return { error: "compound commands are not allowed" };
      push();
      continue;
    }
    if (";&|<>()".includes(char) || char === "#") return { error: "shell control flow and redirects are not allowed" };
    token += char;
    tokenStarted = true;
  }
...
function allowGit(args) {
  ...
  if (READ_ONLY_GIT.has(subcommand)) return !rest.some((arg) => arg === "--output" || arg.startsWith("--output="));
...
export function evaluateAcVerifierCommand(command) {
  ...
  if (executable === "git") return allowGit(args) ? { allowed: true } : denied("Git command is outside the AC-verifier allowlist");
  if (executable === "gh") return allowGh(args) ? { allowed: true } : denied("GitHub command is outside the AC-verifier allowlist");
