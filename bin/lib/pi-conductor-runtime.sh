#!/usr/bin/env bash
# Build an isolated Pi agent/config overlay and policy cwd for the conductor seat.
# This prevents a permissive project-local permission config in the caller's cwd from overriding
# the tracked conductor policy. The caller cwd remains available as the coordination-note root.
set -euo pipefail
...
pi_conductor_prepare_runtime() {
  ...
  if [ -z "$package_path" ] || [ ! -d "$package_path" ]; then
    echo "pi-conductor: @gotgenes/pi-permission-system is not installed; run setup.sh" >&2
    return 1
  fi
  ...
  ln -sfn "$fleet_root/permission-system/conductor.json" \
